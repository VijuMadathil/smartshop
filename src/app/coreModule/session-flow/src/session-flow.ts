 ...
 * 6. Saves user clicks
 *
 * To enable `Session Flow` for tracking inject `DAL` and `Session Flow` into root `AppComponent`.
...
export class SessionFlow {
  ...
  constructor(protected router: Router,
    // @Inject('isBrowser') protected isBrowser
    ) {
    this.userId = 'guest';
    // if (isBrowser) {
   ...
   */
  createVisitedRoute() {
    const visitedRoute: VisitedRoute = new VisitedRoute();
  ...
  addUserClick(e) {
    ...
    const targetValues = ['id', 'name', 'className', 'innerText'];
    userClick.selectorName = 'undefined';
    for (const targetVal in targetValues) {
